...
export function runA11yCheck(): A11yResult[] {
    ...
    function analyzeTables(doc: Document, url: string): A11yResult[] {
        ...
        try {
            ...
            tables.forEach((table) => {
                ...
                results.push({
                    fullUrl: url,
                    type: "Table Info",
                    details: `Caption: ${tableCaption}, Row headers: ${rowHeaders ? "yes" : "no"
    ...
    function extractPdfLinks(doc: Document, url: string): A11yResult[] {
        const results: A11yResult[] = [];
        try {
            const links = doc.querySelectorAll("a[href]");
            links.forEach((link) => {
    ...
    function findOfficeLinks(doc: Document, url: string): A11yResult[] {
        const results: A11yResult[] = [];
        try {
            const links = doc.querySelectorAll("a[href]");
            links.forEach((link) => {
