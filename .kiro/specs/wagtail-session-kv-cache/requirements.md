...
This feature implements Redis caching infrastructure using Upstash Redis to cache Wagtail session state validation results. Currently, the Airtable proxy endpoint validates Wagtail sessions by making HTTP requests to the Wagtail admin interface for each request. By caching these validation results in Upstash Redis, we can reduce latency, decrease load on the Wagtail server, and improve the overall performance of the extension's API interactions.
...
- **Upstash Redis**: A serverless Redis database service integrated with Vercel via the Marketplace, providing Redis-compatible key-value storage for serverless functions
- **Redis Client**: The `@upstash/redis` SDK used to interact with the Upstash Redis database
...
**User Story:** As a system administrator, I want the caching system to handle failures gracefully, so that temporary Redis outages do not break the extension functionality

#### Acceptance Criteria

1. IF THE Upstash Redis service is unavailable, THEN THE Proxy Endpoint SHALL fall back to direct Wagtail session validation
...
4. THE Proxy Endpoint SHALL return successful responses even when Redis operations fail
5. WHEN Redis operations fail, THE Proxy Endpoint SHALL log error details for monitoring and debugging
...
5. THE Proxy Endpoint SHALL include error handling for all Redis operations with descriptive error messages
...
4. WHEN a Redis operation fails, THE Proxy Endpoint SHALL log the error type and error message
